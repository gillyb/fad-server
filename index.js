...
function getDailyFact(event, context) {
  const today = new Date();
  const date = today.getFullYear() + '-' + (today.getMonth() < 9 ? '0' + (today.getMonth()+1) : (today.getMonth()+1)) + '-' + (today.getDate() <= 9 ? '0' + today.getDate() : today.getDate());

  s3.getObject({ Bucket: 'fad-server', Key: date }, (err, file) => {
    if (err) {
      console.log('Error retrieving todays file: ' + err + '. Key: ' + date);
    }

    const todaysFacts = JSON.parse(file.Body.toString());
    context.done(null, todaysFacts);
  });
}
